...
class MainSpec extends AsyncFreeSpec with AsyncIOSpec with Matchers {
  ...
  "Main" - {
    ...
    "should print correct quote for --quote-0237405c-5e85-4f80-b2fc-6560b4fd1802 flag" in {
      captureSystemOut(Main.run(List("--quote-0237405c-5e85-4f80-b2fc-6560b4fd1802"))).asserting { case (exitCode, output) =>
        exitCode shouldBe cats.effect.ExitCode.Success
        output shouldBe "\"Try to be a rainbow in someone's cloud.\" - Maya Angelou"
      }
    }
    
    "should print correct quote for --quote-94da9e08-63e0-49f1-9324-af68ea81018d flag" in {
      captureSystemOut(Main.run(List("--quote-94da9e08-63e0-49f1-9324-af68ea81018d"))).asserting { case (exitCode, output) =>
        exitCode shouldBe cats.effect.ExitCode.Success
        output shouldBe "\"The greatest glory in living lies not in never falling, but in rising every time we fall.\" - Nelson Mandela"
      }
    }
    
    "should print correct quote for --quote-b24de4e6-c9a6-479e-b5ea-21ecba9723bc flag" in {
      captureSystemOut(Main.run(List("--quote-b24de4e6-c9a6-479e-b5ea-21ecba9723bc"))).asserting { case (exitCode, output) =>
        exitCode shouldBe cats.effect.ExitCode.Success
